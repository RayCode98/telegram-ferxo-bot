...
class UserInterest(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", "interest_code", name="uq_user_interest"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
    interest_code: Mapped[str] = mapped_column(String(30), index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

class Favorite(Base):
    __tablename__ = "favorites"
    __table_args__ = (UniqueConstraint("user_id", "favorite_user_id", name="uq_favorite_pair"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
    favorite_user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

class ExperiencePreference(Base):
...
class SubscriptionState(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "product_code", name="uq_subscription_user_product"),
    )
...
class PaymentRefund(Base):
    ...
    __table_args__ = (
        UniqueConstraint("telegram_payment_charge_id", name="uq_payment_refund_charge"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    star_transaction_id: Mapped[str] = mapped_column(
        ForeignKey("star_transactions.id"), index=True
    )
    user_id: Mapped[str] = mapped_column(ForeignKey("users.id"), index=True)
...
class RecoveryRun(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    recovered_conversations: Mapped[int] = mapped_column(Integer, default=0)
    closed_conflicts: Mapped[int] = mapped_column(Integer, default=0)
