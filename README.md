...
## v1.9.6 — Embudo de conversión administrativo

`/admin` incluye ahora `📉 Embudo` y también puede abrirse con `/funnel`.

El reporte trabaja por cohortes de nuevos usuarios de los últimos 7 o 30 días:

1. Entraron al bot.
2. Completaron el perfil.
3. Iniciaron una búsqueda.
4. Consiguieron match.
5. Mandaron su primer mensaje.
6. Obtuvieron una conversación real (ambos participantes escribieron).

Además muestra:

- retención D1 (retorno después de 24 horas, sólo entre usuarios ya elegibles);
- compradores y Stars generadas por la cohorte;
- mayor fuga entre pasos;
- filtros para todos, hombres, mujeres y campañas de adquisición.

El filtro por campaña utiliza la atribución `campaign_attributions`; no mezcla
usuarios de campañas distintas. No requiere nuevas tablas ni migración Alembic.


## v1.9.7 — Monetization Analytics + Contextual Paywalls

### Nuevo `/admin → 🧲 Monetización`
Mide usuarios únicos por etapa durante 7/30 días:

```text
Paywall mostrado
→ Pulsó comprar
→ Factura preparada
→ Pre-checkout aprobado
→ Compra completada
```

También muestra rendimiento por trigger (Likes, filtro de edad, distancia,
límite de búsquedas y menú Premium), Premium Preview otorgados y compradores
por producto.

### Paywalls contextuales
FreXo ya no depende únicamente de una tienda genérica. Los bloqueos Premium
muestran la razón concreta por la que el usuario podría beneficiarse de pagar.
No se inventan likes, urgencia ni escasez.

### Premium Preview
Después de la primera conversación real (ambos enviaron al menos un mensaje),
un usuario elegible recibe una única prueba Premium por 24 horas.

Variables:

```env
PREMIUM_PREVIEW_ENABLED=true
PREMIUM_PREVIEW_HOURS=24
```

### FreXo Pass
Nuevo producto `frexo_pass_7d`: 69 Stars, Premium por 7 días como compra única,
sin renovación automática.

### Base de datos
No requiere migración Alembic nueva. Reutiliza `analytics_events`, `orders`,
`order_contexts`, `star_transactions` y `users.premium_until`.
